...
char          msg[21]       = " MERRY CHRISTMAS ";    // BLE 20 char limit + NUL  DEFAULT MESSAGE
uint8_t       msgLen        = 17;              // DEFAULT message length
int           msgX          = matrix.width(); // Start off right edge
unsigned long prevFrameTime = 0L;             // For animation timing
#define FPS 16                                // Scrolling speed

//**********multiline******************
char          msgtemp[21];
String Smsg = " MERRY XMAS ";
...
void loop() {
  ...
  if((state == ACI_EVT_CONNECTED) && BTLEserial.available()) {
    ...
    } else if (BTLEserial.peek() == ':') { // ****************Multiline********************
      msgLen      = readStr(msgtemp, sizeof(msgtemp)-1);
      switch (msgtemp[1]) {
      case 0x5E: // "^" denotes new string
        Smsg = "";
        break;
      case 0x2B: // "+"
        Smsg += msgtemp+2;
